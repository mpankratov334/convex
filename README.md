...
---
### Постановка задачи модификации
Вычисляется сумма квадратов расстояний от начала координат до середин рёбер 
выпуклой оболочки.

### Решение задачи модификации

- В класс `R2Point` добавлен метод возвращающий квадрат расстояния от начала координат до точки 
- В класс `Figure` добавлен метод возвращающий квадрат суммы расстояний как ноль
- В класс `Segment` добавлен метод, который находит середину ребра и возвращает квадрат расстояния от это точки
- Класс `Polygon` теперь при инициализации рассчитывает искомую величину как сумму для всех его отрезков-двуугольников
- Метод `add` класса `Polygon` теперь пересчитывает вклад каждого отрезка в искомую величину подобно пересчёту периметра
